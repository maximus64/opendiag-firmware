...
#define TEST_FAIL(msg) \
    td_fail(__FILE__, __LINE__, "%s", (msg))
...
#define TEST_ASSERT_MSG(cond, ...) do {                                     \
        if (!(cond)) {                                                      \
...
#define TEST_ASSERT_EQUAL_MEM(expected, actual, len) do {                   \
        ...
        if (__builtin_memcmp(td_e_, td_a_, td_n_) != 0) {                   \
            td_fail(__FILE__, __LINE__, "%s:\n    expected [%s]\n"          \
                    "    actual   [%s]", #actual,                           \
                    td_hex(td_e_, td_n_), td_hex(td_a_, td_n_));            \
        }                                                                   \
    } while (0)
