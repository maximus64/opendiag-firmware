...
static int  port0_write(const void *b, uint32_t n) { return port_write(0, b, n); }
static int  port1_write(const void *b, uint32_t n) { return port_write(1, b, n); }
...
static const comm_port_ops_t g_ops[FAKE_PORT_COUNT] = {
    { .write = port0_write, .flush = port0_flush, .is_connected = port0_connected },
    { .write = port1_write, .flush = port1_flush, .is_connected = port1_connected },
};
...
void fake_port_register_all(void)
{
    ...
    for (int i = 0; i < FAKE_PORT_COUNT; i++) {
        g_port[i].id = comm_port_register(g_names[i], &g_ops[i],
                                          FAKE_PORT_RX_BUF);
    }
}
