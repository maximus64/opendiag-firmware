...
static int j1850_pwm_tx_controlled(const bus_msg_t *msg, uint32_t flags,
                                   bus_tx_control_t *control) {
    ...
    for (uint8_t attempt = 0;; attempt++) {
        ...
        /* Prepare the expected echo; arm it only after bus admission. */
        __atomic_store_n(&g.echo_armed, false, __ATOMIC_RELEASE);
        memcpy(g.echo, frame, len + 1u);
        g.echo_len = len + 1u;

        rc = tx_once(g.tx_symbols, n, control);
        if (rc != 0) {
            break;
        }
...
static int j1850_pwm_tx(const bus_msg_t *msg, uint32_t flags) {
    return j1850_pwm_tx_controlled(msg, flags, NULL);
}
...
const bus_ops_t j1850_pwm_bus_ops = {
    ...
    .send = j1850_pwm_tx,
    .send_controlled = j1850_pwm_tx_controlled,
    .recv = j1850_pwm_rx,
